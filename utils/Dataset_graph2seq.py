...
from utils.util_graph2seq.nlp_utils import split_chinese_sentence, remove_stopwords
from utils.util_graph2seq.dict_utils import cosine_sim
from utils.util_graph2seq.utils import bow
...
class DataLoader:
    def __init__(self, config, data_path, batch_size, vocab, adj_type, use_gnn, model, no_train=False, debug=False, ):
        ...
        if no_train:
            batch_size = config.max_generator_batches

        # 训练数据
        if not no_train:
            self.train_data = self.read_json(os.path.join(data_path, 'dev_graph_features.json'), adj_type, is_train=True, use_gnn=use_gnn)
            self.train_batches = self.make_batch(self.train_data, batch_size, is_train=True, model=model)
            random.shuffle(self.train_batches)

        # 验证和测试数据
        self.dev_data = self.read_json(os.path.join(data_path, 'dev_graph_features.json'), adj_type, is_train=True, use_gnn=use_gnn)
        self.dev_batches = self.make_batch(self.dev_data, batch_size, is_train=True, model=model)

        self.test_data = self.read_json(os.path.join(data_path, 'test_graph_features.json'), adj_type, is_train=False, use_gnn=use_gnn)
